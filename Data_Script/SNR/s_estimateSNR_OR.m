...
for i = 1:length(subjDir)
    ...
    % Load OT ROI
    l_or_ap = niftiRead(fullfile(baseDir, subjDir{i}, 'dwi', 'fibers','OR_orig','L_OR_AP.nii.gz'));
    r_or_ap = niftiRead(fullfile(baseDir, subjDir{i}, 'dwi', 'fibers','OR_orig','R_OR_AP.nii.gz'));
    l_or_pa = niftiRead(fullfile(baseDir, subjDir{i}, 'dwi', 'fibers','OR_orig','L_OR_PA.nii.gz'));
    r_or_pa = niftiRead(fullfile(baseDir, subjDir{i}, 'dwi', 'fibers','OR_orig','R_OR_PA.nii.gz'));
 
    or_roi_ap = l_or_ap.data + r_or_ap.data;
    or_roi_pa = l_or_pa.data + r_or_pa.data;
    ...
    % Find OT voxels
    [r,c,v] = ind2sub(size(or_roi_ap), find(or_roi_ap==1));
    ...
    % Find OT voxels
    [r,c,v] = ind2sub(size(or_roi_pa), find(or_roi_pa==1));
...
savefilename = 'b0SNR_estimate_OR.mat';
